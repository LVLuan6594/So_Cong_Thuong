...
// Bộ lọc lớp cho bản đồ GIS ở đầu trang: nhóm "Lưới điện" gộp 4 cấp điện áp.
const ENERGY_MAP_LAYER_OPTIONS: EnergyMapLayerOption[] = [
  { label: "Trạm điện", keys: ["substations"] },
  { label: "Lưới điện", keys: ["lines500", "lines220", "lines110", "lines22"] },
  { label: "Trụ điện", keys: ["poles"] },
  { label: "Nguồn năng lượng tái tạo", keys: ["projects"] },
  { label: "Điện mặt trời mái nhà", keys: ["rooftopSolar"] },
  { label: "Vùng phụ tải", keys: ["keyConsumers"] },
  { label: "Điểm sự cố", keys: ["incidents"] },
  { label: "Trạm sạc điện", keys: ["chargingStations"] },
];

const ENERGY_MAP_INITIAL_LAYERS: EnergyMapLayerKey[] = [
  "substations",
  "lines500",
  "lines220",
  "lines110",
...
function Page() {
  ...
  const gisSectionRef = useRef<HTMLElement>(null);
  const gisHeaderRef = useRef<HTMLElement>(null);
  const [gisFullscreen, setGisFullscreen] = useState(false);
  const [fsMapHeight, setFsMapHeight] = useState(580);

  useEffect(() => {
    const onFullscreenChange = () =>
      setGisFullscreen(document.fullscreenElement === gisSectionRef.current);
    document.addEventListener("fullscreenchange", onFullscreenChange);
    return () => document.removeEventListener("fullscreenchange", onFullscreenChange);
  }, []);

  // Khi fullscreen: đo chiều cao còn lại của viewport để cấp chiều cao pixel tuyệt đối cho bản đồ
  // (tránh phụ thuộc chuỗi phần trăm height trong flex — nguyên nhân gây màn hình trắng).
  useEffect(() => {
    if (!gisFullscreen) {
      setFsMapHeight(580);
      return;
    ...
    const update = () => {
      const headerH = gisHeaderRef.current?.offsetHeight ?? 58;
      setFsMapHeight(Math.max(320, window.innerHeight - headerH - 36));
    };
    update();
  ...
  const toggleGisFullscreen = () => {
    const el = gisSectionRef.current;
    if (!el) return;
  ...
  const scopedSubstations = useMemo(
    () =>
      filterByDistrict(substationsQuery.data ?? [], district).sort(
        (a, b) => (b.loadFactor ?? 0) - (a.loadFactor ?? 0),
      ),
    [district, substationsQuery.data],
  );
  const scopedProjects = useMemo(
    () => filterByDistrict(projectsQuery.data ?? [], district),
    [district, projectsQuery.data],
  );
  const activeIncidents = useMemo(
    () =>
      (incidentsQuery.data ?? [])
        .filter((i) => district === "Toàn tỉnh" || (i.affectedArea ?? "").includes(district))
        .filter((i) => !["Hoàn thành", "Đã xử lý"].includes(i.progress ?? "")),
    [district, incidentsQuery.data],
  );
  ...
  const mapSelectedKey = selectedMapEntity
    ? `${selectedMapEntity.kind}:${selectedMapEntity.item.id}`
    : aiFocus?.kind === "substation"
      ? `substation:${aiFocus.id}`
      : null;
    ...
    <>
      ...
      <div className="space-y-5 p-4 sm:p-6">
        ...
        <section
          ref={gisSectionRef}
          className={cn(
            "rounded-2xl border border-teal/25 bg-teal/[0.08] p-3 sm:p-4",
            gisFullscreen &&
              "fixed inset-0 z-[1000] flex h-screen w-screen flex-col overflow-auto rounded-none",
          )}
        >
          <section
            className={cn(
              "gov-card overflow-hidden",
              gisFullscreen && "flex min-h-0 flex-1 flex-col",
            )}
          >
            <header
              ref={gisHeaderRef}
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-0.5 text-xs text-muted-foreground">
                  Hiển thị và khai thác các lớp dữ liệu năng lượng trên địa bàn tỉnh.
                </p>
              </div>
              <Button
                variant="ghost"
                size="sm"
                onClick={toggleGisFullscreen}
                className="text-gov"
                title={gisFullscreen ? "Thoát toàn màn hình" : "Phóng to bản đồ"}
              >
                {gisFullscreen ? <Minimize className="size-4" /> : <Maximize className="size-4" />}
                <span className="sr-only">
                  {gisFullscreen ? "Thoát toàn màn hình" : "Phóng to bản đồ"}
                </span>
              </Button>
            </header>
            <div className={cn(gisFullscreen && "min-h-0 flex-1")}>
              <EnergyMap
                data={gisData}
                height={fsMapHeight}
                fill={gisFullscreen}
                layerOptions={ENERGY_MAP_LAYER_OPTIONS}
                initialLayers={ENERGY_MAP_INITIAL_LAYERS}
          ...
          <div className="grid grid-cols-1 gap-4 2xl:grid-cols-[minmax(0,1fr)_290px]">
            <div className="space-y-4">
              ...
              <section className="grid grid-cols-1 gap-4 lg:grid-cols-2 xl:grid-cols-3">
                ...
                <ChartCard title="Tình hình sự cố" className="lg:col-span-2 xl:col-span-1">
